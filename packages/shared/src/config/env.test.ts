...
describe("loadConfig", () => {
  ...
  it("accepts the full sector basket", () => {
    const config = loadConfig({
      ENABLE_LIVE_TRADING: "false",
      SYMBOLS: "BTCUSDT,BNBUSDT,DOGEUSDT,ZECUSDT,TAOUSDT,FETUSDT,ONDOUSDT,AAVEUSDT,INJUSDT,AXSUSDT,BERAUSDT,ENSOUSDT,PENGUUSDT,PAXGUSDT,XAUTUSDT,LINKUSDT,PYTHUSDT,ARBUSDT,OPUSDT,RENDERUSDT,AKTUSDT,LDOUSDT,EIGENUSDT,TIAUSDT,DYMUSDT,UNIUSDT,JUPUSDT,FILUSDT,ARUSDT,TONUSDT,NOTUSDT,ICPUSDT,NEARUSDT,DOTUSDT,AVAXUSDT,ATOMUSDT,GRTUSDT,STXUSDT,IMXUSDT,POLUSDT,ROSEUSDT,ENAUSDT,PENDLEUSDT,HBARUSDT,THETAUSDT,AUDIOUSDT,HYPEUSDT,SNXUSDT,DYDXUSDT,RUNEUSDT,AXLUSDT,MKRUSDT,SKYUSDT,ENSUSDT,BLURUSDT,SANDUSDT,GALAUSDT,SHIBUSDT,PEPEUSDT,WIFUSDT,APEUSDT,SEIUSDT,SUIUSDT"
    });
    ...
    expect(config.SYMBOLS).toContain("ICPUSDT");
    expect(config.SYMBOLS).toContain("PENDLEUSDT");
  });
