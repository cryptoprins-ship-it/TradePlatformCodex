...
describe("loadConfig", () => {
  ...
  it("accepts the full sector basket", () => {
    const config = loadConfig({
      ENABLE_LIVE_TRADING: "false",
      SYMBOLS: "BTCUSDT,BNBUSDT,DOGEUSDT,ZECUSDT,TAOUSDT,FETUSDT,ONDOUSDT,AAVEUSDT,INJUSDT,AXSUSDT,BERAUSDT,ENSOUSDT,PENGUUSDT,GOLD(PAXG)USDT,LINKUSDT,PYTHUSDT,ARBUSDT,OPUSDT,RENDERUSDT,LDOUSDT,EIGENUSDT,TIAUSDT,DYMUSDT,UNIUSDT,JUPUSDT,FILUSDT,ARUSDT,TONUSDT,NOTUSDT,ICPUSDT,NEARUSDT,DOTUSDT,AVAXUSDT,ATOMUSDT,GRTUSDT,STXUSDT,IMXUSDT,POLUSDT,ROSEUSDT,ENAUSDT,PENDLEUSDT,HBARUSDT,HYPEUSDT,SNXUSDT,DYDXUSDT,RUNEUSDT,SKYUSDT,ENSUSDT,BLURUSDT,SANDUSDT,GALAUSDT,SHIBUSDT,PEPEUSDT,WIFUSDT,APEUSDT,SEIUSDT,SUIUSDT"
    });

    expect(config.SYMBOLS).toContain("AAVEUSDT");
    expect(config.SYMBOLS).toContain("PENGUUSDT");
    expect(config.SYMBOLS).toContain("GOLD(PAXG)USDT");
